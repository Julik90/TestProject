...
public class DevClass {
    ...
    public static boolean logIn(String login, String password, String conirmPassword) throws WrongLoginException, WrongPasswordException {
        if (!(login.length() < 20 && isValidLogin(login))) {
            throw new WrongLoginException();
        }

        if (!(password.length() < 20 && isValidPassword(password) && password.equals(conirmPassword))) {
